            ...
            <div class="p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="md:col-span-2">
                            <div class="grid grid-cols-1 gap-4">
                                <div>
                                    <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400">البريد الإلكتروني</h4>
                                    <p class="mt-1 text-gray-900 dark:text-white">{{ $profile->email }}</p>
                                </div>
                                <div>
                                    <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400">الهاتف</h4>
                                    <p class="mt-1 text-gray-900 dark:text-white">{{ $profile->phone ?? 'غير متوفر' }}</p>
                                </div>
                                <div>
                                    <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400">نبذة</h4>
                                    <p class="mt-1 text-gray-900 dark:text-white">{{ $profile->bio ?? 'غير متوفر' }}</p>
                                </div>
                                <div>
                                    <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400">السيرة الذاتية</h4>
                                    @if ($profile->cv_path)
                                        <div class="mt-1">
                                    ...
                                    @else
                                        <p class="mt-1 text-gray-900 dark:text-white">لم يتم رفع سيرة ذاتية</p>
                                    @endif
                                </div>
                            </div>
                        </div>
                ...
                @else
                    <div class="text-center py-8">
                        ...
                        <h3 class="mt-2 text-lg font-medium text-gray-900 dark:text-white">لا يوجد ملف شخصي</h3>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">أنشئ ملفًا شخصيًا لعرض معلوماتك.</p>
                        <div class="mt-6">
                            @can('portfolio.edit')
                                <a href="{{ route('admin.portfolio.profile.edit') }}" class="btn-primary">
                                    إنشاء ملف شخصي
