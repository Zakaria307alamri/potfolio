            ...
            <div class="p-6">
                                            ...
                                            <div class="flex justify-end space-x-2">
                                                ...
                                                @can('portfolio.delete')
                                                    <form action="{{ route('admin.portfolio.experiences.destroy', $experience) }}" method="POST" class="inline-block">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-300" onclick="return confirm('هل أنت متأكد من حذف هذه الخبرة؟')">
                                                            حذف
                                                        </button>
                                                    </form>
                                                @endcan
                ...
                @else
                    <div class="text-center py-8">
                        ...
                        <h3 class="mt-2 text-lg font-medium text-gray-900 dark:text-white">لا توجد خبرات</h3>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">ابدأ بإضافة أول خبرة.</p>
                        <div class="mt-6">
                            @can('portfolio.create')
                                <a href="{{ route('admin.portfolio.experiences.create') }}" class="btn-primary">
                                    إضافة خبرة
